...
void calculate(GtkButton *button, gpointer data)
{
	...
	if (strcmp("=", text) == 0) 
	{
		...
		if (add) for (int i = 0; i < x; i++) result += num[i];
		if (divv) result = num[0] / num[1];
		if (sub) 
		{
		if (result == 0.0) result = num[0] * 2;
		for (int i = 0; i < x; i++) result -= num[i];
		}
		if (mul) result = num[0] * num[1];
	...
	else 
	{
		...
		else strncat(input_buffer, text, 1);
